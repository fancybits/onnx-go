...
func init() {
	register("ReduceMean", newReduceMean)
}
...
func newReduceMean() operator {
	return &reduceMean{
		keepdims: true, // default
	}
}

func (r *reduceMean) apply(g *Graph, ns ...*Node) error {
	n := ns[0]
	children := getOrderedChildren(g.g, n)

	// ReduceMean can have 1 input (old opset, axes as attribute) or 2 inputs (new opset, axes as tensor)
	...
	for _, axis := range axes {
		result, err = gorgonia.Mean(result, axis)
		if err != nil {
			return err
		}
	}
...
func (r *reduceMean) init(o onnx.Operation) error {
